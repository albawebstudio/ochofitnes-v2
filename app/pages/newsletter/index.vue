...
<template>
  <div class="w-full bg-white dark:bg-gray-900 pb-24">
    ...
    <div class="max-w-7xl bg-white rounded-3xl shadow-md -mt-20 mx-auto p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
      <div v-for="item in newsletters" :key="item._path" class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex flex-col">
        <a href="#">
        ...
        <div class="p-5 flex flex-col flex-grow items-start">
          ...
          <!-- Use localePath with the converted route path -->
          <NuxtLink :to="localePath(getRoutePath(item._path))" class="mt-auto inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            {{ newsletter.cta }}
